...
if not TURSO_DATABASE_URL or not TURSO_AUTH_TOKEN:
    raise EnvironmentError("❌ TURSO_DATABASE_URL dan TURSO_AUTH_TOKEN harus di-set!")

try:
    import libsql_client as libsql
except ImportError:
    raise ImportError("❌ pip install libsql-client")
...
class TogelPredictor:
    ...
    def _get_digit_counter(self, position: Optional[str] = None) -> Counter:
        ...
        idx_map = {'AS': 0, 'KOP': 1, 'KEPALA': 2, 'EKOR': 3}
    ...
    def get_ct_3d(self) -> str:
        ...
        if len(recent) < 10: recent = self.results
    ...
    def get_strong_pairs(self):
        ...
        pairs = Counter()
        for num in self.results[:80]:
            for i in range(4):
                for j in range(i+1, 4):
                    pair = ''.join(sorted([num[i], num[j]]))
                    pairs[pair] += 1
